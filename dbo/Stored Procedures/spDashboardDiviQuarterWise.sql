CREATE procedure [dbo].[spDashboardDiviQuarterWise]
--exec [spDashboardDiviQuarterWise] 1,8,2019
(
	@Distributor int,
	@Month int ,
	@Year int
...
,(case when (isnull(sum(l.MonthlyTrgtValue),0))=0 then 0 else CONVERT(decimal(12,2),(sum(isnull((case when month(a.SalesFromDate) = @Month then a.NetTotValues end),'0')))*100/isnull(l.MonthlyTrgtValue,0)) end) as 'CurMthAch(%)'
